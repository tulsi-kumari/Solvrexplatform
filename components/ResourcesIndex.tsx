...
const styles = `
  ...
  .sx-res-card { display: flex; flex-direction: column; padding: 22px; border-radius: 10px; }
          ...
          <div className="sx-res-grid">
            {RESOURCES.map((a) => (
              <Link key={a.slug} href={`/resources/${a.slug}`} className="sx-res-card" aria-label={a.title}>
                ...
                <p style={{ fontSize: "14px", color: C.textMuted, lineHeight: 1.6, marginBottom: "20px", flex: 1 }}>{a.description}</p>
